...
LRESULT WINAPI ScreenSaverProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_TIMER:
    {
        ...
        for (wLoop = 0; wLoop < wDensity; wLoop++)
        {
            ...
            nYTemp = static_cast<int>((nY[wLoop] * SCOPE * WARPFACTOR) / DIVIDE_SAFE(nZ[wLoop]))
                                                     + wY2Screen;
            nTemp = static_cast<int>((SCOPE * WARPFACTOR - nZ[wLoop]) /
                                                    (SIZE * WARPFACTOR)) + 1;
            ...
            nXTemp = static_cast<int>((nX[wLoop] * static_cast<LONG>(SCOPE * WARPFACTOR))
                                                    / DIVIDE_SAFE(nZ[wLoop])) + wX2Screen;
            nYTemp = static_cast<int>((nY[wLoop] * SCOPE * WARPFACTOR)
    ...
    case WM_ERASEBKGND:
        ...
        if (fill_bkgrnd) {
          ...
          FillRect (reinterpret_cast<HDC>(wParam), &rRect, static_cast<HBRUSH>(GetStockObject (GRAY_BRUSH)));
          return 0l;
        }
        break;
    case WM_DESTROY:
...
WINBOOL WINAPI ScreenSaverConfigureDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_INITDIALOG:
        ...
        if (hIDOK != nullptr) {
          ...
          SendDlgItemMessage( hDlg, ID_DENSITYARROW, UDM_SETBUDDY, reinterpret_cast<WPARAM>(hDensity), 0);
    ...
    case WM_HELP: // F1
        WinHelp(
            static_cast<HWND>(reinterpret_cast<LPHELPINFO>(lParam)->hItemHandle),
            szHelpFile,
            HELP_WM_HELP,
...
VOID CreateStar(WORD wIndex) {
    nX[wIndex] = wXScreen ? static_cast<LONG>(static_cast<int>(ZRAND (wXScreen)) - static_cast<int>(wX2Screen)) : 0;
    nY[wIndex] = wXScreen ? static_cast<LONG>(static_cast<int>(ZRAND (wYScreen)) - static_cast<int>(wY2Screen)) : 0;
...
VOID GetIniEntries(VOID) {
    ...
    wWarpSpeed = static_cast<WORD>(GetPrivateProfileInt (szAppName, szWarpSpeed, MINWARP + ((MAXWARP - MINWARP) / 2), szIniFile));
